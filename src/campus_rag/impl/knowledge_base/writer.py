...
async def _construct_embedding_key(chunk: dict, chunk_keys: list[str], max_value_size: int):
    embedding_keys = ""
...
async def upload(collection_name: str, knowledge: list[dict], chunk_keys: list[str], max_value_size: int, meta_field: bool)\
        -> bool:
    try:
...
async def modify_chunk_by_id(collection_name: str, chunk_id: int, new_chunk: str) -> bool:
    try:
